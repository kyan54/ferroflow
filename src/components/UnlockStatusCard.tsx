...
export function UnlockStatusCard({ results, busy, error, running, onCheck }: UnlockStatusCardProps) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{t.dashboard.unlock.title}</CardTitle>
        <Button variant="secondary" size="sm" busy={busy} onClick={onCheck} disabled={!running}>
          {t.dashboard.unlock.checkButton}
        </Button>
      </CardHeader>
      <CardContent className="pt-4">
        {!running ? (
          <p className="text-sm text-fg-faint">{t.dashboard.unlock.notRunning}</p>
        ) : error ? (
          <p className="rounded-md border border-err/30 bg-err-weak px-3 py-2 text-sm text-err">{error}</p>
        ) : !results ? (
          <p className="text-sm text-fg-faint">{t.dashboard.unlock.explainer}</p>
        ) : (
            ...
            {results.map((r) => (
              ...
              >
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                  <Badge variant={STATUS_BADGE[r.status]}>{t.dashboard.unlock.statusLabels[r.status]}</Badge>
